...
Energyの自然生成位置とEnemyの出現位置は、共通の生成角度設定でXZ平面上の扇形へ限定できます。`CenterAngleDegrees`は扇形の中心方向（0度が+X、90度が+Z）、`RangeDegrees`は0～360度の角度幅です。既定値の360度では全周、180度では指定方向を中心とする半円から生成します。Enemy撃破時のEnergyドロップ位置はこの制限を受けません。
...
- 自然生成される同時存在上限は12個です。
- 生成位置は各領域の円環とフィールドの生成角度範囲内で、面積に対して一様になるよう抽選します。
...
- 同時存在上限は20体です。
- FieldのOuterBuffer円周上かつフィールドの生成角度範囲内からランダムに出現します。
- 現在の移動速度は毎秒1.0です。
