...
float3 Blur1D_9Tap(float2 uv, float2 uvStepSize, float2 direction)
{
    ...
    w[3] = exp(-9.0f * rcpTwoSd2);
    w[4] = exp(-16.0f * rcpTwoSd2);
...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    output.color.rgb = Blur1D_9Tap(input.texcoord, uvStepSize, float2(0.0f, 1.0f));
    output.color.a = 1.0f;
    return output;
}